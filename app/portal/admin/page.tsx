...
export default function SaaSAdminDashboard() {
        ...
        const init = async () => {
            try {
                ...
                if (!storedUser) {
                    if (mounted) {
                        setLoading(false);
                        // Redirect handled by render return
                ...
                if (userData.role !== 'SAAS_OWNER') {
                    if (mounted) {
                        setLoading(false);
                        router.replace('/portal/pro');
                    }
                    return;
                }

                // 3. Set User & Fetch Data
    ...
    // LOADING STATE - Prevents layout shift/flash
    if (loading) {
        return (
            <div className="min-h-screen bg-slate-950 flex items-center justify-center">
                <div className="flex flex-col items-center gap-4">
                    <Loader2 className="h-10 w-10 text-primary animate-spin" />
                    <p className="text-slate-500 font-mono text-xs uppercase tracking-widest">Initializing Control Plane...</p>
                </div>
            </div>
        );
    }

    // AUTH GUARD - Redirect Fallback
    if (!user) {
            ...
            <CrashGuard name="Sidebar" fallback={<div className="w-72 bg-slate-900 border-r border-slate-800 hidden md:flex items-center justify-center">Sidebar Error</div>}>
                <aside className="w-72 bg-slate-900 border-r border-slate-800 flex flex-col fixed h-full z-20 hidden md:flex">
                    ...
                    <nav className="flex-1 px-4 space-y-1 mt-6">
                        <AdminNavItem href="/portal/admin" icon={<BarChart3 size={20} />} label="Global Overview" active />
                        <AdminNavItem href="/portal/admin/tenants" icon={<Building2 size={20} />} label="Tenant Management" badge={tenants?.length?.toString() || '0'} />
                        <AdminNavItem href="/portal/admin/audit" icon={<Activity size={20} />} label="System Audit" />
                        <AdminNavItem href="/portal/admin/security" icon={<ShieldCheck size={20} />} label="Security Policies" />
                        <AdminNavItem href="/portal/admin/settings" icon={<Globe size={20} />} label="Global Settings" />
                    </nav>
...
function AdminNavItem({ icon, label, active = false, badge, href }: any) {
    return (
        <a href={href || '#'} className={`flex items-center gap-3 px-5 py-3.5 rounded-2xl transition-all ${active
