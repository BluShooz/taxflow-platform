        ...
        <div className="min-h-screen bg-slate-950 text-slate-100 flex font-sans">
            ...
            <aside className="w-72 bg-slate-900 border-r border-slate-800 flex flex-col fixed h-full z-20">
                ...
                <nav className="flex-1 px-4 space-y-1 mt-6">
                    ...
                    <AdminNavItem icon={<Building2 size={20} />} label="Tenant Management" badge={tenants.length.toString()} />
            ...
            <main className="flex-1 ml-72">
                <header className="bg-slate-950/50 backdrop-blur-xl px-12 py-6 flex items-center justify-between sticky top-0 z-10 border-b border-white/5">
                    <div className="flex items-center gap-6 bg-slate-900 px-5 py-2.5 rounded-2xl w-full max-w-xl border border-slate-800 focus-within:border-primary/50 transition-colors">
                                        ...
                                        <tbody className="divide-y divide-slate-800">
                                            {tenants.map((tenant) => (
                                                <tr key={tenant.id} className="hover:bg-slate-800/40 transition-all cursor-pointer group">
                                                    ...
                                                    <td className="px-8 py-6">
                                                        <div className="text-xs font-bold text-slate-300">{tenant._count.users} Users</div>
                                                        <div className="text-[10px] text-slate-600 font-medium">{tenant._count.files} Documents</div>
                                                    </td>
                                    ...
                                    {stats?.recentActivity?.map((log: any) => (
                                        <div key={log.id} className="relative pl-10 group">
                                            ...
                                            <div className="flex flex-col gap-1">
                                                ...
                                                <p className="text-xs font-bold text-slate-200">{log.action.replace(/_/g, ' ')}</p>
                                                <p className="text-[10px] text-slate-600 font-medium">
                                                    {log.user?.email.split('@')[0]} • {new Date(log.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                                                </p>
                                            </div>
                                        </div>
                                    ))}
